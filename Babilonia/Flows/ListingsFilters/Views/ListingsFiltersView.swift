...
final class ListingsFiltersView: NiblessView {

    let listingTypeView: FiltersListingTypeView = .init()
    let histogramView: FiltersHistogramView = .init()
    let totalAreaRangeView: FiltersSliderRangeView = .init()
    let builtAreaRangeView: FiltersSliderRangeView = .init()
    let yearRangeView: FiltersYearRangeView = .init()
    let counterView: FiltersCounterView = .init()
    private let checkmarksView: FiltersCheckmarksView = .init()
    private let facilitiesView: FiltersFacilitiesView = .init()
    ...
    private var facilitiesHeightConstraint: NSLayoutConstraint?
    private var countersHeightConstraint: NSLayoutConstraint?
    private var checkmarksHeightConstraint: NSLayoutConstraint?
    ...
    func updateCheckmarks(with viewModels: [CreateListingFacilityViewModel]) {
        checkmarksView.updateCheckmarks(with: viewModels)
    }

    func updateFacilities(with viewModels: [CreateListingFacilityViewModel]) {
        facilitiesView.updateFacilities(with: viewModels)
    }

    func setupHistogram(with values: [HistogramValue]) {
        histogramView.setupHistogram(with: values)
    }

    func updateCounters(with viewModels: [NumberFieldViewModel]) {
        counterView.updateCounters(with: viewModels)
    }

}

extension ListingsFiltersView {
