    ...
    class FirebaseDistributionSetupAction < Action
      ...
      def self.run(params)
        ...
        if !File.exist?(credentials_path) then
          UI.error("No Google Application Credentials has been found at \"#{credentials_path}\". Make sure that either have set either :google_application_credentials or :firebase_setup_keys_dir")
          return
        end        
      ...
      def self.read_plist_value(info_plist_path, key)
        `/usr/bin/defaults read \"#{info_plist_path}\" \"#{key}\"`.strip
      end
